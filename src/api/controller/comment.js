...
module.exports = class extends Base {
  ...
  async countAction() {
    ...
    const allCount = await this.model('comment').where({ value_id: valueId }).count('id');

    const hasPicCount = await this.model('comment').where({ type_id: 1, value_id: valueId }).count('id');
