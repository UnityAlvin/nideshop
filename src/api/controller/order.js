...
module.exports = class extends Base {
  ...
  async submitAction() {
    ...
    switch (state) {
      case 'cart':
        checkedGoodsList = await this.model('cart').where({ user_id: think.userId, session_id: 1, checked: 1 }).select();
        ...
        break;
      case 'buyNow':
        checkedGoodsList = this.post('checkedGoodsList');
        if (think.isEmpty(checkedGoodsList)) {
          return this.fail('请选择商品');
        }
        goodsTotalPrice += checkedGoodsList[0].number * checkedGoodsList[0].retail_price;
        break;
    }
  ...
  async deleteAction() {
    ...
    const order = await this.model('order').where({ id: orderId }).delete();
